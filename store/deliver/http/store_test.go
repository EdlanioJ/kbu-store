...
func Test_StoreHandler_Active(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			app := fiber.New()
			http.NewStoreRoute(app, storeUsecase)
			req := httptest.NewRequest(fiber.MethodOptions, fmt.Sprintf("/stores/active/%s", tc.arg), nil)
			req.Header.Set("Content-Type", "application/json")
			res, err := app.Test(req)
			tc.checkResponse(t, err, res.StatusCode)
			storeUsecase.AssertExpectations(t)
...
func Test_StoreHandler_Block(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			app := fiber.New()
			http.NewStoreRoute(app, storeUsecase)
			req := httptest.NewRequest(fiber.MethodOptions, fmt.Sprintf("/stores/block/%s", tc.arg), nil)
			req.Header.Set("Content-Type", "application/json")
			res, err := app.Test(req)
			tc.checkResponse(t, err, res.StatusCode)
			storeUsecase.AssertExpectations(t)
...
func Test_StoreHandler_Disable(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			app := fiber.New()
			http.NewStoreRoute(app, storeUsecase)
			req := httptest.NewRequest(fiber.MethodOptions, fmt.Sprintf("/stores/disable/%s", tc.arg), nil)
			req.Header.Set("Content-Type", "application/json")
			res, err := app.Test(req)
			tc.checkResponse(t, err, res.StatusCode)
			storeUsecase.AssertExpectations(t)
