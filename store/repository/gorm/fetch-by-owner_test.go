...
func Test_GormFetchStoreByOwnerRepository(t *testing.T) {
	is := assert.New(t)
	db, mock, store := testMock()

	repo := gorm.NewGormFetchStoreByOwner(db)
	page := 2
	limit := 10
	...
	list, total, err := repo.Exec(context.TODO(), store.ExternalID, sort, limit, page)
