...
func Test_GormFetchStoreByStautsRepository(t *testing.T) {
	is := assert.New(t)
	db, mock, store := testMock()

	repo := gorm.NewGormFetchStoreByStauts(db)
	page := 2
	limit := 10
	...
	list, total, err := repo.Exec(context.TODO(), store.Status, sort, limit, page)
